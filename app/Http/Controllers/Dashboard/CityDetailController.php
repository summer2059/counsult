...
class CityDetailController extends Controller
{
    ...
    public function __construct(CrudService $crudService)
    {
        ...
        $this->modelName = 'CityDetail';
    }

    public function index()
    {
        $cd = CityDetail::first(); 
        return view('dashboard.city-banner.index', compact('cd'));
    }
    ...
    public function update(ConsultRequest $request)
    {
        ...
        if ($request->id) {
            try {
                ...
                toast('City Banner updated successfully', 'success');
        ...
        try {
            ...
            toast('City Banner added successfully', 'success'); 
            return back()->withInput();
        } catch (\Exception $e) {
            Log::error('Error: ' . $e->getMessage(), ['exception' => $e, 'trace' => $e->getTraceAsString()]);
