...
class CityController extends Controller
{
            ...
            try {
                ...
                return DataTables::of($data)
                    ...
                    ->addColumn('action', function ($data) {
                        return '<a href="/dashboard/city/' . $data->id . '/edit" class="btn btn-sm btn-primary"> Edit</a>
                                <a href="/dashboard/city/' . $data->id . '" class="btn btn-sm btn-danger" data-confirm-delete="true"> Delete</a>';
                    })
                    ->rawColumns(['action', 'image'])
                    ->make(true);
            } catch (\Exception $e) {
                Log::error('City index error: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
    ...
    public function create()
    {
        try {
            ...
            return view('dashboard.city.create', compact('categories'));
        } catch (\Exception $e) {
    ...
    public function store(Request $request)
    {
        try {
            ...
            Log::error('City store error: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return back()->withErrors(['error' => 'Failed to save City.'])->withInput();
        }
    }
    ...
    public function edit(string $id)
    {
        try {
            ...
            return back()->withErrors(['error' => 'Failed to load city.']);
        }
    }
    ...
    public function update(Request $request, string $id)
    {
        try {
            ...
            Log::error('City update error: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return back()->withErrors(['error' => 'Failed to update city.'])->withInput();
        }
    }
