    ...
    <div id="kt_app_content_container" class="app-container container-xxl">
        <div class="card">
            ...
            <div class="card-body pt-0 mt-4">
                <form action="{{ route('faqs.store') }}" method="POST" enctype="multipart/form-data" id="careerForm">
                    @csrf
                    ...
                    <div class="col-12 mb-3 lang-field lang-english">
                        <label for="title">Question (English)</label>
                    ...
                    <div class="col-12 mb-3 lang-field lang-japanese d-none">
                        <label for="jp_question">Question (Japanese)</label>
                        <input type="text" class="form-control" name="jp_question" value="{{ old('jp_question') }}">
                        @error('jp_question')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <!-- Description Fields -->
                    <div class="col-12 mb-3 lang-field lang-english">
                        <label for="description">Answer (English)</label>
                        <textarea name="answer" class="form-control summernote">{{ old('answer') }}</textarea>
                        @error('answer')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 mb-3 lang-field lang-japanese d-none">
                        <label for="description">Answer (Japanese)</label>
                        <textarea name="jp_answer" class="form-control summernote">{{ old('jp_answer') }}</textarea>
                        @error('jp_answer')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
