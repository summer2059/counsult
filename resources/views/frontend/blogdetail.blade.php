...
<div class="container-fluid bg-dark p-5">
    <div class="row">
        <div class="col-12 text-center">
            ...
            <a href="{{ route('blog.detail', $blog->slug) }}">Blog Detail</a>
        </div>
    </div>
</div>
...
<div class="container-fluid py-6 px-5">
    <div class="row g-5">
        <div class="col-lg-8">
            <div class="mb-5">
                <img class="img-fluid w-100 mb-5" src="{{ asset('uploads/images/' . $blog->image) }}" alt="{{ $blog->title }}">
                ...
                <div class="d-flex flex-column justify-content-start bg-secondary p-4">
                    @foreach($categories as $category)
                        <a class="h5 mb-3" href="{{ route('blog', ['category' => $category->slug]) }}">
                            <i class="bi bi-arrow-right text-primary me-2"></i>{{ $category->title }}
                        </a>
                    @endforeach
                </div>
            </div>
                ...
                @foreach($recentBlogs as $item)
                    <div class="d-flex mb-3">
                        <img class="img-fluid" src="{{ asset('uploads/images/' . $item->image) }}"
                             style="width: 100px; height: 100px; object-fit: cover;" alt="{{ $item->title }}">
                        <a href="{{ route('blog.detail', $item->slug) }}"
                           class="h5 d-flex align-items-center bg-secondary px-3 mb-0">{{ $item->title }}</a>
                    </div>
                @endforeach
            </div>
        </div>
