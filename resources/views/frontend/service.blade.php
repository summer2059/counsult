    ...
    @if ($service->isNotEmpty())
        @foreach ($service as $sev)
            <div class="col-lg-4 col-md-6">
                <a href="{{ route('service.detail', $sev->slug) }}" class="text-decoration-none">
                    <div class="service-item bg-secondary text-center px-5 text-dark">
                        <div class="d-flex align-items-center justify-content-center bg-primary text-white rounded-circle mx-auto mb-4"
                            ...
                            <img src="{{ asset('uploads/images/' . $sev->image) }}" alt="{{ $sev->title }}"
                                style="width: 100%; height: 100%; object-fit: cover;">
                        </div>

                        <h3 class="mb-3">{{ $sev->title }}</h3>
                        <p class="mb-0 text-dark">{!! Str::limit(strip_tags($sev->description), 100) !!}</p>
                    </div>
                </a>
            </div>
        @endforeach
