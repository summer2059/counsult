                ...
                <div class="bg-white shadow-sm rounded p-4">
                    ...
                    <ul class="list-group list-group-flush" id="service-list">
                        @foreach ($nonMenuServices as $service)
                            <li class="list-group-item list-hover cursor-pointer" data-title="{{ $service->title }}"
                ...
                @foreach ($menuServices as $service)
                    <div class="col-md-4">
                        <div class="card menu-card shadow-sm border-0" data-bs-toggle="modal" data-bs-target="#menuModal"
                            data-image="{{ asset('uploads/images/' . $service->image) }}" data-title="{{ $service->title }}"
                            data-description="{{ e($service->description) }}"
                            data-price="{{ number_format($service->price, 2) }}">
                            <img src="{{ asset('uploads/images/' . $service->image) }}" class="card-img-top"
